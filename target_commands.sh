...
# It does alot of checking to make sure nothing went wrong and shoots of an email if somethings a bit off.
#
# REQUIREMENTS:
# mail, and hostname commands, as well as rsync and bash must be available. Also on Fbsd add :/usr/local/bin to the end of the path in crontab.
#
#
# *****************************************************
...
# *inital release
# 1.0.1
# *rewrite the rsync check function so it wont fail on an exit code of 24 (Partial transfer due to vanished source files) and echos rsync codes to the log file.
# *change log path to a variable defined in user define section.
# *change interpreter to /usr/bin/env bash and added path declaration for portability.
# *added support for checking mysql dumps/rotations (check check_sqldump_mv).
# *script tries to continue when part of the actual backup fails (moving files, dumps, rsync, etc). originally it would cry a bit and exit.
# 1.0.2
# *include target_commands.sh in same directory. This contains functions which contain the backup commands. This is to make this script cleaner and keep eliminates the need to touch this script.
# *create last status file which has the date that this script last ran. This will be in /tmp on each server and have its own line to transfer the file to the backup server.
# 1.0.2
...
# just get the path the script is in
script_dir=`dirname "$fullpath"`

#include file that contains the backup commands.
# if the file does not exist then exit.
if [ -f $script_dir/target_commands.sh ] ; then
...
# NOTE: old versions of hostname dont support -f
...
#Prep work done, we attempt to start the backup

#Lets check to see if this script is already running
#small segment below is adapted from http://www.askdavetaylor.com/shell_script_test_to_see_if_its_already_running.html

if [ -f /tmp/backup_script.lock ] ; then

# the lock file already exists, so check to see if the pid is valid
if [ "$(ps -p `cat /tmp/backup_script.lock` | wc -l)" -gt 1 ]; then
